...
test('entries.get: GETs /v1/entries/:id and returns the parsed entry', async (t) => {
  ...
  const got = await pact.entries.get('a7f2bcde-412')
  t.is(m.calls[0].url, 'http://127.0.0.1:7666/v1/pacts/default/entries/a7f2bcde-412')
  t.alike(got, entry)
})
...
test('entries.referencedBy: GETs the referenced-by suffix', async (t) => {
  ...
  const pact = new OpenPact({ fetch: m.fetch, pactId: 'default' })
  await pact.entries.referencedBy('a7f2bcde-412')
  t.is(m.calls[0].url, 'http://127.0.0.1:7666/v1/pacts/default/entries/a7f2bcde-412/referenced-by')
})
