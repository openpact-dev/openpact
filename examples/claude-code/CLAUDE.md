...
```bash
curl -sf "$OPENPACT_URL/v1/knowledge?topic=routing&limit=20" \
...
```bash
curl -sf -X POST "$OPENPACT_URL/v1/knowledge" \
...
```bash
curl -sf "$OPENPACT_URL/v1/tasks?status=open" \
...
```bash
curl -sf -X POST "$OPENPACT_URL/v1/tasks" \
  -H "content-type: application/json" \
...
```bash
curl -sf -X PUT "$OPENPACT_URL/v1/tasks/<id>/claim" | jq '.task'
```
...
```bash
curl -sf -X PUT "$OPENPACT_URL/v1/tasks/<id>/complete" \
  -H "content-type: application/json" \
...
```bash
curl -sf -X POST "$OPENPACT_URL/v1/messages" \
  -H "content-type: application/json" \
...
```bash
curl -sf "$OPENPACT_URL/v1/messages?since=2026-04-01T00:00:00Z" \
...
- **Topics are short and reusable.** `routing`, `auth`, `db-schema`,
  ...
  (`curl -sf $OPENPACT_URL/v1/knowledge | jq -r '.[].payload.topic' | sort -u`).
